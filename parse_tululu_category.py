...
def get_end_page_to_parse(url):
    ...
    end_page = soup.select('.npage')[-1]
    return end_page


def parse_category_for_book_urls(start_page, end_page):
    book_urls = []
    base_url = 'https://tululu.org/l55/'
    if not end_page:
        end_page = get_end_page_to_parse(base_url)
    for page_to_parse in range(start_page, end_page+1):
        with suppress(requests.HTTPError):
            url = f'{base_url}{page_to_parse}/'
            response = requests.get(url)
            response.raise_for_status()
            soup = BeautifulSoup(response.text, 'lxml')
...
def main():
    ...
    book_urls = parse_category_for_book_urls(args.start_page, args.end_page)
    downloaded_books = []
